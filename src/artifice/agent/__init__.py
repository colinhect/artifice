...
def create_agent(config: ArtificeConfig, on_connect: Callable | None = None) -> AgentBase | None:
    ...
    if provider.lower() == "ollama":
        return OllamaAgent(model=model_name, system_prompt=config.system_prompt, thinking_budget=thinking_budget, on_connect=on_connect)
    elif provider.lower() == "anthropic":
        return ClaudeAgent(model=model_name, system_prompt=config.system_prompt, thinking_budget=thinking_budget, on_connect=on_connect)
    elif provider.lower() == "copilot":
        return CopilotAgent(model=model_name, system_prompt=config.system_prompt, on_connect=on_connect)
